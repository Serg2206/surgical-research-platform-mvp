...
- **CNAME** для `www.proffssv.site` → `85729bb29bbfbe89.vercel-dns-017.com.`
...
| **CNAME** | `www` | `85729bb29bbfbe89.vercel-dns-017.com.` | 300 (или Auto) |

> ⚠️ **Важно:** Если у регистратора уже есть записи типа A или CNAME для `@` или `www`, удалите их перед добавлением новых.
> Для текущей настройки удалить старые A-записи `@` → `185.199.108.153` и `@` → `185.199.109.153`.

> ⚠️ **ВАЖНО:** Сделайте скриншот текущей DNS-зоны перед внесением изменений для возможности быстрого отката.
...
Используйте скрипт `check_dns.sh` из корня репозитория (уже создан) или команды `dig`/`nslookup` вручную:
...
# Ожидаемый результат: 85729bb29bbfbe89.vercel-dns-017.com.
...
- [ ] Старые A-записи `@` → `185.199.108.153` и `185.199.109.153` удалены
- [ ] CNAME `www` → `85729bb29bbfbe89.vercel-dns-017.com.` создана у регистратора
...
| 404 на домене (ADM.tools) | Если домен куплен на ADM.tools — проверьте, не включён ли внутренний форвардинг/редирект в панели управления. Это частая причина «404 на домене» при корректных A/CNAME записях |
